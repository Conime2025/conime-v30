...
interface SharePopupProps {
  ...
  title: string;
  url: string;
  image?: string; // Add image preview
}

export function SharePopup({ isOpen, onClose, title, url, image }: SharePopupProps) {
  const { language } = useLanguage();
  const popupRef = useRef<HTMLDivElement>(null);
  const [copied, setCopied] = useState(false);

  // Social media platforms with circle design - 2 columns layout
    ...
    <div className="fixed inset-0 z-50 bg-black/20 backdrop-blur-sm flex items-center justify-center p-4">
      ...
      >
        ...
        <div className="flex items-center justify-between p-6 border-b">
          ...
          <Button variant="ghost" size="sm" onClick={onClose} className="hover:bg-muted rounded-full h-8 w-8 p-0">
        ...
        <div className="p-6 space-y-6">
          ...
          {/* Social Media Platforms - 2 Columns Grid */}
          <div className="grid grid-cols-2 gap-6 justify-items-center">
            {socialPlatforms.map((platform) => (
              ...
              >
                <div className={`
                  w-16 h-16 rounded-full ${platform.bgColor} ${platform.textColor}
          ...
          <div className="space-y-3">
            ...
            {copied && (
              <p className="text-xs text-green-600 text-center">
                {language === 'id' ? 'Link berhasil disalin!' : 'Link copied successfully!'}
              </p>
            )}
          </div>
        </div>
        ...
        <div className="p-4 border-t bg-muted/30 rounded-b-2xl">
          <p className="text-xs text-muted-foreground text-center">
            {language === 'id' 
              ? 'Terima kasih telah membagikan konten Conime!'
              : 'Thank you for sharing Conime content!'
            }
          </p>
        </div>
      </div>
